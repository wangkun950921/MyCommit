...
namespace WindowsFormsApplication1
{
    class Opertion
    {
         ...
         */
        public Boolean BookAdd(string name, string number, string kinds, string buff)
        {
            SqlConnection conn = new SqlConnection(constr);
            try
            {
                conn.Open();
                /*insert into Books(Bname, Bnumber,Bkinds, Bbuff) values('a', '123456', '自然科学', '借出');*/
                string sqlstr = "insert into Books(Bname, Bnumber,Bkinds, Bbuff) values('"
                    + name + "', '"
                    + number + "', '"
                    + kinds +"', '未借出')";
                SqlDataAdapter da = new SqlDataAdapter(sqlstr, conn);
        ...
        public Boolean BookUpdateByNumber(string name, string number, string kinds, string buff)
        {
            SqlConnection conn = new SqlConnection(constr);
            try
            {
        ...
        //根据书的名字修改
        public Boolean BookUpdateByName(string name, string number, string kinds, string buff)
        {
            SqlConnection conn = new SqlConnection(constr);
            try
            {
